...
# Purpose: This script takes the building characteristics of the housing stock existing in 2020 and applies equipment and insulation upgrades in line with the assumptions of the Extensive/Electrification Renovation scenario
...
# Outputs:- Intermediate_results/RenExtElec.RData, projection of the <2020 housing stock to 2060 including all characteristics changes due to renovations.  A large file stored in LF_Data/Intermediate_results
#         - Intermediate_results/ExElRenSummary.RData, summary of changes in <2020 housing stock 2020-2060 by different equipment/efficiency characteristics and total stock numbers
...
# define the matrix for wall insulation switching. If I need, there are higher types of insulation in the options lookup table, in case the current aren't strong enough, which is likely
...
# For the Avd scenario I basically define that insulation switches 25:75 to levels 1:2 levels higher, unless on the second-to-top or top level, in which they can only go 1 or 0 levels higher
...
# again define swithcing 25:75 up 1/2 levels where possible
pcrins[2:3,1]<-c(0.25,0.75)
pcrins[3,2:3]<-1
pcrins[4,4]<-1 # None remains None
pcrins[6:7,5]<-c(0.25,0.75)
pcrins[7,6:7]<-1
...
# R-19 in now the minimum level for new construction, so leapfrog from uninsulated to at least R-19, but at higher rates than in standard renovation
puains[4:5,1:2]<-c(0.5,0.5)
puains[4:6,3]<-c(0.1,0.75,0.15) # higher levels if starting from R-13
puains[5:6,4]<-c(0.25,0.75)
puains[6:7,5]<-c(0.25,0.75)
puains[7:8,6]<-c(0.25,0.75)
puains[8,7:8]<-1
...
# now define probability matrices for switching water heater,  table(rs$Water.Heater). mainly some fuel switching, plus some upgrades to 'premium' efficiency and 'tankless'
...
pctype_all["Room","Room",]<-1-pctype_all["None","Room",]-pctype_all["Central","Room",]
...
ce[3,1:2]<-1
# for central AC SEER == 13, replace with 13:15:18 in ratio 80:20
ce[3:5,3]<-c(0,0.8,0.2)
# for central AC SEER ==15, repace with 15:18 in ratio 05:95
ce[4:5,4]<-c(0.05,0.95)
# central SEER 18, HP, and None all remain the same
ce[5:7,5:7]<-diag(3)
# for 8.5 and 9.8 EER room AC, move up two levels in 25:75 ratio
ce[9:10,8]<-ce[10:11,9]<-c(0.25,0.75)
# for 10.7 EER, replace with 10.7 12.0 in 05:95 ratio
ce[10:11,10]<-c(0.05,0.95)
...
for (yr in 2021:2060) {
  ...
  for (r in 1:4) {# four regions 
    ...
    hren<-AdvSF*pren[RGs[r],"SF","Heat"] # the renovation rate for a given region, type, system combination, adjusted to advanced rate
    ...
    # assign ASHPs to the equivalent 1 and 2 levels higher, in 25:75 ratio, changed in Adv
    for (rc in 1:4) { # for row/column 1:4
      phfe[rc+1:2,rc]<-pfuel_all["Electricity HP","Electricity HP",RGs[r]]*c(0.25,0.75) # multiply the chance of remaining with a heat pump by 0.25, 0.75 as we split this into two efficiency levels
    }
    ...
    # switching from el HP to fuel (only gas applies), assign ducted ASHP to two most efficient furnaces in 25:75 ratio, changed in Adv
    phfe[c("Natural Gas_Fuel Furnace, 80% AFUE","Natural Gas_Fuel Furnace, 92.5% AFUE"),c(1:6)]<-c(0.25,0.75)*pfuel_all["Gas","Electricity HP",RGs[r]]
    # assign non-ducted MSHP, assume 20:20:40:20 split (Adv) between the two most efficient boilers and the most efficient floor/wall furnace ,"Natural Gas_Fuel Boiler, 96% AFUE"
    phfe[c("Natural Gas_Fuel Boiler, 80% AFUE","Natural Gas_Fuel Boiler, 90% AFUE"),c(11:13)]<-0.2*pfuel_all["Gas","Electricity HP",RGs[r]]
    phfe[c("Natural Gas_Fuel Boiler, 96% AFUE"),c(11:13)]<-0.4*pfuel_all["Gas","Electricity HP",RGs[r]]
    ...
    # assign non-ducted resistance electricity to non-ducted MSHP, two least efficient in 25:75 ratio
    phfe[11:12,c(7,8,10)]<-pfuel_all["Electricity HP","Electricity",RGs[r]]*c(0.25,0.75)
    # assign ducted electricity furnaces to two standard efficiency ASHP in 25:75 ratio
    phfe[2:3,9]<-pfuel_all["Electricity HP","Electricity",RGs[r]]*c(0.25,0.75)
    
    # switching from el resistance ducted to fuel ducted, assume a 25:75 split (Adv) between the two most efficient types, this is a much more efficient mix than the current mix of fuel furnaces
    phfe[c("Natural Gas_Fuel Furnace, 80% AFUE","Natural Gas_Fuel Furnace, 92.5% AFUE"),c(9)]<-c(0.25,0.75)*pfuel_all["Gas","Electricity",RGs[r]]
    phfe[c("Fuel Oil_Fuel Furnace, 80% AFUE","Fuel Oil_Fuel Furnace, 92.5% AFUE"),c(9)]<-c(0.25,0.75)*pfuel_all["Oil","Electricity",RGs[r]]
    phfe[c("Propane_Fuel Furnace, 80% AFUE","Propane_Fuel Furnace, 92.5% AFUE"),c(9)]<-c(0.25,0.75)*pfuel_all["Propane","Electricity",RGs[r]]
    # switching from el res non-ducted to gas non-ducted, assume 20:20:40:20 split (Adv) between the three most efficent boilers and  most efficient floor/wall furnace
    phfe[c("Natural Gas_Fuel Boiler, 80% AFUE","Natural Gas_Fuel Boiler, 90% AFUE"),c(7,8,10)]<-0.2*pfuel_all["Gas","Electricity",RGs[r]]
    phfe[c("Natural Gas_Fuel Boiler, 96% AFUE"),c(7,8,10)]<-0.4*pfuel_all["Gas","Electricity",RGs[r]]
    phfe[c("Natural Gas_Fuel Wall/Floor Furnace, 68% AFUE"),c(7,8,10)]<-0.2*pfuel_all["Gas","Electricity",RGs[r]]
    # switching from el res non-ducted to non-gas fuel non-ducted, assume 25:75:0 split (Adv) between the two most efficent boilers and  most efficient floor/wall furnace
    phfe[c("Fuel Oil_Fuel Boiler, 80% AFUE","Fuel Oil_Fuel Boiler, 90% AFUE"),c(7,8,10)]<-c(0.25,0.75)*pfuel_all["Oil","Electricity",RGs[r]]
    phfe[c("Fuel Oil_Fuel Wall/Floor Furnace, 68% AFUE"),c(7,8,10)]<-0*pfuel_all["Oil","Electricity",RGs[r]]
    phfe[c("Propane_Fuel Boiler, 80% AFUE","Propane_Fuel Boiler, 90% AFUE"),c(7,8,10)]<-c(0.25,0.75)*pfuel_all["Propane","Electricity",RGs[r]]
    phfe[c("Propane_Fuel Wall/Floor Furnace, 68% AFUE"),c(7,8,10)]<-0*pfuel_all["Propane","Electricity",RGs[r]]
    
    # Renovations of oil heating systems
    # Oil->Oil
    # oil boilers go up (or most efficient stay the same), in 25:75 (Adv) ratio
    phfe[16:17,15]<-pfuel_all["Oil","Oil",RGs[r]]*c(0.25,0.75)
    phfe[17,16:17]<-pfuel_all["Oil","Oil",RGs[r]]
    # oil ducted furnaces go up (or most efficient stay the same)
    phfe[19:20,18]<-pfuel_all["Oil","Oil",RGs[r]]*c(0.25,0.75)
    ...
    # Oil->Elec HP
    # Non-ducted oil heaters go to  MSHP, with a  25:75 (Adv) split of the two most efficient
    phfe[c(11:12),c(15:17,21:22)]<-pfuel_all["Electricity HP","Oil",RGs[r]]*c(0.25,0.75)
    # ducted oil furnaces go to low-mid efficiency ASHP, with a 25:75 (Adv) split
    phfe[c(2:3),18:20]<-pfuel_all["Electricity HP","Oil",RGs[r]]*c(0.25,0.75)
    # Oil->Elec Res
    ...
    # Oil->Gas
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv) split
    phfe[30:31,18:20]<-pfuel_all["Gas","Oil",RGs[r]]*c(0.25,0.75)
    ...
    # Oil->Propane
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv) split
    phfe[43:44,18:20]<-pfuel_all["Propane","Oil",RGs[r]]*c(0.25,0.75)
    # non-ducted go to non-ducted of the same type, but up an efficiency level
    ...
    phfe[25:26,24]<-pfuel_all["Gas","Gas",RGs[r]]*c(0.25,0.75)
    phfe[26:27,25]<-pfuel_all["Gas","Gas",RGs[r]]*c(0.6,0.4) # make it less likely to change up to a 96% than 90% efficient, from 80%
    ...
    phfe[30:31,29]<-phfe[29:30,28]<-pfuel_all["Gas","Gas",RGs[r]]*c(0.25,0.75)
    ...
    # Gas->Elec HP
    # Non-ducted gas heaters go to MSHP, with a  25:75 (Adv) split to the low:mid efficiency options
    phfe[c(11:12),c(24:27,32:33)]<-pfuel_all["Electricity HP","Gas",RGs[r]]*c(0.25,0.75)
    # ducted gas furnaces go to low-mid efficiency ASHP, with a 25:75 (Adv) split
    phfe[c(2:3),28:31]<-pfuel_all["Electricity HP","Gas",RGs[r]]*c(0.25,0.75)
    # Gas -> Elec Res
    ...
    # Gas->Oil
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv) split
    phfe[19:20,28:31]<-pfuel_all["Oil","Gas",RGs[r]]*c(0.25,0.75)
    ...
    # Gas->Propane
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv) split
    phfe[43:44,28:31]<-pfuel_all["Propane","Gas",RGs[r]]*c(0.25,0.75)
    # non-ducted go to non-ducted of the same type, but up an efficiency level
    ...
    phfe[39:40,38]<-pfuel_all["Propane","Propane",RGs[r]]*c(0.25,0.75)
    ...
    phfe[43:44,42]<-phfe[42:43,41]<-pfuel_all["Propane","Propane",RGs[r]]*c(0.25,0.75)
    ...
    # Propane->Elec HP
    # Non-ducted prop heaters go to  MSHP, with a  25:75 (Adv) split
    phfe[c(11:12),c(38:40,45:46)]<-pfuel_all["Electricity HP","Propane",RGs[r]]*c(0.25,0.75)
    # ducted prop furnaces go to electric ducted furnaces or low-mid efficiency ASHP, with a 25:75 (Adv) split
    phfe[c(2:3),41:44]<-pfuel_all["Electricity HP","Propane",RGs[r]]*c(0.25,0.75)
    ...
    # Propane->Oil
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv) split
    phfe[19:20,41:44]<-pfuel_all["Oil","Propane",RGs[r]]*c(0.25,0.75)
    # non-ducted go to non-ducted of the same type, but up an efficiency level
    phfe[22,45:46]<-phfe[17,39:40]<-phfe[16,38]<-pfuel_all["Oil","Propane",RGs[r]]
    # Propane->Gas
    # furnaces go to furnaces (two most efficient) with 25:75 (Adv)0 split
    phfe[30:31,41:44]<-pfuel_all["Gas","Propane",RGs[r]]*c(0.25,0.75)
    # non-ducted go to non-ducted of the same type, but up an efficiency level
    ...
    wren<-AdvSF*pren[RGs[r],"SF","Water"] # the renovation rate for a given region, type, system combination
    ...
    # assign electric standard to el standard/prem/tankless/HP with 75:15:5:5 (adv) odds
    whfe[1:4,"Electric Standard"]<-c(0.75,0.15,0.05,0.05)*pwfuel_all["Electricity","Electricity",RGs[r]]
    # assign electric standard to gas standard/prem/tankless with 75:10:15 odds, rougly based on https://www.eia.gov/analysis/studies/residential/pdf/res_ee_fuel_switch.pdf pg 14
    whfe[5:7,"Electric Standard"]<-c(.75,.1,.15)*pwfuel_all["Gas","Electricity",RGs[r]]
    ...
    # assign electric standard to propane standard/prem/tankless with 75:10:15 odds, rougly based on https://www.eia.gov/analysis/studies/residential/pdf/res_ee_fuel_switch.pdf pg 71
    whfe[12:14,"Electric Standard"]<-c(.75,.1,.15)*pwfuel_all["Propane","Electricity",RGs[r]]
    # assign elec premium to elec premium/tankless/HP, with 30:35;35 odds, 
    whfe[2:4,"Electric Premium"]<-c(0.3,0.35,0.35)*pwfuel_all["Electricity","Electricity",RGs[r]]
    # assign elec premium to elec premium/tankless/HP, with 0:50:50 odds, 
    whfe[2:4,"Electric Tankless"]<-c(0,0.5,0.5)*pwfuel_all["Electricity","Electricity",RGs[r]]
    # assign elec hp to elec hp
    whfe[2:4,"Electric Heat Pump, 80 gal"]<-c(0,0,1)*pwfuel_all["Electricity","Electricity",RGs[r]]
    # elec premium/tankless/HP to gas standard/prem/tankless with 0:65:35 odds
    whfe[5:7,2:4]<-c(0,.65,.35)*pwfuel_all["Gas","Electricity",RGs[r]]
    # Define values in technology/efficiecy/fuel switching probability matrix phfe.
    # assign electric premium/tankless/HP to propane standard/prem/tankless with 0:65:35 odds
    whfe[12:14,2:4]<-c(0,.65,.35)*pwfuel_all["Propane","Electricity",RGs[r]]
    ### renovations of gas water heating systems ###
    # assign gas standard to gas standard/prem/tankless with  40:25:35 odds
    whfe[5:7,"Natural Gas Standard"]<-c(0.4,0.25,0.35)*pwfuel_all["Gas","Gas",RGs[r]]
    # gas standard to elec with  75:10:5:10 odds
    whfe[1:4,"Natural Gas Standard"]<-c(0.75,0.1,0.05,0.1)*pwfuel_all["Electricity","Gas",RGs[r]]
    # all gas to oil standard
    whfe[8,5:7]<-pwfuel_all["Oil","Gas",RGs[r]]
    # gas stnard to propane standard/prem/tankless with 75:10:15 odds
    whfe[12:14,"Natural Gas Standard"]<-c(.75,.1,.15)*pwfuel_all["Propane","Gas",RGs[r]]
    ...
    # gas prem/tankless to elec prem/tankless/HP with 30:35;35 odds
    whfe[2:4,6:7]<-c(0.3,0.35,0.35)*pwfuel_all["Electricity","Gas",RGs[r]]
    ...
    # oil standard to elec with 75:15:5:5 odds
    whfe[1:4,"Fuel Oil Standard"]<-c(0.75,0.15,0.05,0.05)*pwfuel_all["Electricity","Oil",RGs[r]]
    # oil standard to gas standard/prem/tankless with 75:10:15 odds
    whfe[5:7,"Fuel Oil Standard"]<-c(.75,.1,.15)*pwfuel_all["Gas","Oil",RGs[r]]
    # oil standard to propane standard/prem/tankless with 75:10:15 odds
    whfe[12:14,"Fuel Oil Standard"]<-c(.75,.1,.15)*pwfuel_all["Propane","Oil",RGs[r]]
    ...
    # assign propane standard to propane standard/prem/tankless with 40:25:35 odds
    whfe[12:14,"Propane Standard"]<-c(0.4,0.25,0.35)*pwfuel_all["Propane","Propane",RGs[r]]
    # propane standard to elec with  75:15:5:5 odds
    whfe[1:4,"Propane Standard"]<-c(0.75,0.15,0.05,0.05)*pwfuel_all["Electricity","Propane",RGs[r]]
    # all propane to oil standard
    whfe[8,12:14]<-pwfuel_all["Oil","Propane",RGs[r]]
    # propane standard to gas standard/prem/tankless with 75:10:15 odds
    whfe[5:7,"Propane Standard"]<-c(.75,.1,.15)*pwfuel_all["Gas","Propane",RGs[r]]
    ...
    # propane prem/tankless to elec prem/tankless/HP with 30:35;35odds
    whfe[2:4,13:14]<-c(0.3,0.35,0.35)*pwfuel_all["Electricity","Propane",RGs[r]]
    ...
    iren<-AdvSF*pren[RGs[r],"SF","Ins"] # the renovation rate for a given region, type, system combination
    ...
    for (i in 1:nrow(SFirensam)) {
      ...
      if (!SFirensam$Insulation.Wall_New[i]==SFirensam$Insulation.Wall[i] | # if any of these have been changed,,
          ...
          !SFirensam$Infiltration_New[i]==SFirensam$Infiltration[i]) {SFirensam$change_iren[i]<-yr} # make note if the insulation actually changes
    }
    ...
    ctr<-pctype_all[,,RGs[r]] # rate of changing between no/room/central AC cooling type, this isn't increased in the Adv scenario
    ...
    cren<-AdvSF*pren[RGs[r],"SF","Cool"] # chance of changing efficiency as defined in ce, only for those with room/central AC
    ...
    hren<-AdvSF*pren[RGs[r],"MF","Heat"] # the renovation rate for a given region, type, system combination
    hLT<-round(1/hren) # average lifetime of equipment, or period for replacements
    hIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(hLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-hLT # age at which adoption rate matches the full rate
    ...
    # same for water heating fuel and efficiency
    wren<-AdvSF*pren[RGs[r],"MF","Water"] # the renovation rate for a given region, type, system combination
    wLT<-round(1/wren) # average lifetime of equipment, or period for replacements
    wIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(wLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-wLT # age at which adoption rate matches the full rate
    ...
    iren<-AdvSF*pren[RGs[r],"MF","Ins"] # the renovation rate for a given region, type, system combination
    iLT<-round(1/iren) # average lifetime of equipment, or period for replacements
    iIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(iLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-iLT # age at which adoption rate matches the full rate
    ...
    # next implement efficiency changes in space cooling
    cren<-AdvSF*pren[RGs[r],"MF","Cool"] # chance of changing efficiency as defined in ce, only for those with room/central AC
    cLT<-round(1/cren) # average lifetime of equipment, or period for replacements
    cPC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(cLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-cLT # age at which adoption rate matches the full rate
    ...
    hren<-AdvSF*pren[RGs[r],"MH","Heat"] # the renovation rate for a given region, type, system combination
    hLT<-round(1/hren) # average lifetime of equipment, or period for replacements
    hIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(hLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-hLT # age at which adoption rate matches the full rate
    ...
    # same for water heating fuel and efficiency
    wren<-AdvSF*pren[RGs[r],"MH","Water"] # the renovation rate for a given region, type, system combination
    wLT<-round(1/wren) # average lifetime of equipment, or period for replacements
    wIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(wLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-wLT # age at which adoption rate matches the full rate
    ...
    iren<-AdvSF*pren[RGs[r],"MH","Ins"] # the renovation rate for a given region, type, system combination
    iLT<-round(1/iren) # average lifetime of equipment, or period for replacements
    iIC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(iLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-iLT # age at which adoption rate matches the full rate
    ...
    # next implement efficiency changes in space cooling
    cren<-AdvSF*pren[RGs[r],"MH","Cool"] # chance of changing efficiency as defined in ce, only for those with room/central AC
    cLT<-round(1/cren) # average lifetime of equipment, or period for replacements
    cPC<-data.frame("Year"=1900:yr,"Rate"=0) # implementation curve
    half<-yr-(cLT/2) # half the lifetime, after which adoption increases from zero
    full<-yr-cLT # age at which adoption rate matches the full rate
...
save(rs_2020_2060,file="../LF_Data/Intermediate_results/RenExtElec.RData")
...
for (sts in 1:4) {
  ...
  for (yr in c(seq(2025,2060,5))) {
    dd<-get(paste("rs",yr,sep = "_")) # i need to have all of these saved. Not doing this for now, due to space and questionable necessity
...
ggplot(df,aes(x=Year,y=1e-6*Count,fill=HeatFuel))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme_bw() + 
  labs(title = "Pre-2020 housing units by main heat fuel, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(wfe,aes(x=Year,y=1e-6*Count,fill=WaterFuel_Efficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme_bw() + scale_fill_manual(values = cols)+
  labs(title = "Pre-2020 housing units by main water heating system, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(hfe_el,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma)+ theme_bw() +scale_fill_manual(values = cols) +
  labs(title = "Electric heating systems by equipment type and efficiency, 2020-2060",  y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(hfe_gas,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme_bw()  + scale_fill_manual(values = cols) +
  labs(title = "Gas heating systems by equipment type and efficiency, 2020-2060",  y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(hfe_oil,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma)+ theme_bw() + scale_fill_manual(values = cols) +
  labs(title = "Oil heating systems by equipment type and efficiency, 2020-2060",y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(ins,aes(x=Year,y=1e-6*Count,fill=WallInsulation))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme_bw()  + scale_fill_manual(values = cols)+  #scale_fill_brewer(palette="Paired") +
  labs(title = "Pre-2020 housing units by Wall Insulation, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))
...
ggplot(cte,aes(x=Year,y=1e-6*Count,fill=AC.Type))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma) + theme_bw()+ scale_fill_manual(values = cols) +
  labs(title = "Pre-2020 housing units by AC Type, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Electrification Renovation",sep=""))

save(df_base,df_hiDR,hfe_base,hfe_hiDR,ins_base,ins_hiDR,cte_base,cte_hiDR,wfe_base,wfe_hiDR,file="../Intermediate_results/ExElRenSummary.RData")
